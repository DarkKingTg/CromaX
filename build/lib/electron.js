...
async function main(arch = process.arch) {
    ...
    const exeName = process.platform === 'win32' ? (product.nameShort + '.exe') : product.nameShort;
    const targetExe = path_1.default.join(electronPath, exeName);
    if (fs_1.default.existsSync(targetExe)) {
        return;
    }
    if (!fs_1.default.existsSync(electronPath)) {
        fs_1.default.mkdirSync(electronPath, { recursive: true });
    }
    try {
        await util.streamToPromise(getElectron(arch)());
    }
    catch (e) {
        console.warn('gulp-electron stream warning:', e);
    }
    if (process.platform === 'win32') {
        const electronExe = path_1.default.join(electronPath, 'electron.exe');
        if (fs_1.default.existsSync(electronExe) && !fs_1.default.existsSync(targetExe)) {
            fs_1.default.copyFileSync(electronExe, targetExe);
        }
    }
    const versionFile = path_1.default.join(electronPath, 'version');
    if (!fs_1.default.existsSync(versionFile)) {
        fs_1.default.writeFileSync(versionFile, electronVersion);
    }
}
